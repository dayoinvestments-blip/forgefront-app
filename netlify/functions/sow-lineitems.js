...
async function fetchFullDescription(description_url) {
  ...
  try {
    ...
    if (ct.includes('application/json')) {
      ...
      // SAM.gov description endpoint variants
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // If inline text is short, try fetching full description from SAM.gov
    if (sow.length < 500 && description_url && description_url.startsWith('http')) {
      ...
      if (full && full.length > sow.length) {
        ...
        console.log('Full description fetch returned nothing useful; using inline text');
      }
    }
    ...
    if (sow.length < 50) {
      return {
        statusCode: 400,
        headers: CORS,
        body: JSON.stringify({ error: 'SOW text too short to extract line items.' }),
      };
    }
  ...
  "items": [
    {
      ...
      "spec": "specific requirement or standard from the SOW (regulation, code, grade, or operational requirement)",
